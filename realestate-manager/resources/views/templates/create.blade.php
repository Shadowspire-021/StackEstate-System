<x-app-layout>
    <x-slot name="header">
        <h2 class="font-extrabold text-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent leading-tight">
            {{ __('Create Installment Plan Template') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12 bg-gray-50/50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('templates.store') }}" method="POST" class="space-y-6" x-data="templateForm()">
                @csrf
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Standard Monthly Plan" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2" placeholder="Brief description of this template..." class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('description') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Duration (Months)</label>
                        <input type="number" name="duration_months" value="{{ old('duration_months', 12) }}" min="1" max="360" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6" x-show="type === 'graduated' || type === 'balloon'">
                    ...
                    <div x-show="type === 'graduated'" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="config[start_percentage]" value="{{ old('config.start_percentage', 50) }}" min="1" max="100" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            ...
                            <div>
                                ...
                                <input type="number" name="config[increment_percentage]" value="{{ old('config.increment_percentage', 10) }}" min="0" max="100" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    ...
                    <div x-show="type === 'balloon'" class="space-y-4">
                        <div>
                            ...
                            <input type="number" name="config[balloon_percentage]" value="{{ old('config.balloon_percentage', 40) }}" min="1" max="90" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
